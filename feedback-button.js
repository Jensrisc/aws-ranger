(function () {
    ...
    button.textContent = 'Upload Feedback';
    button.style = 'margin: 6px; padding: 8px; background: #0073bb; color: white; border: none; border-radius: 4px;';
    button.onclick = () => {
        const redirectTarget = `https://${window.location.host}/s3/upload/ranger-production?region=eu-west-1&bucketType=general&prefix=feedback/`;
    ...
    function waitForContainer(callback) {
        const observer = new MutationObserver(() => {
            ...
            if (target && target.parentElement && !document.querySelector('.cbm-btn-feedback')) {
                observer.disconnect();
